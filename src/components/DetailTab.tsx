...
import { Sparkles, Calendar, ArrowUpRight, ArrowDownRight, Star, RefreshCw, BarChart2, ShieldAlert, Bot, TrendingUp, Layers, Search, X, ChevronRight } from 'lucide-react';
import { StockDetail, Stock } from '../types';
...
export default function DetailTab({
  ...
  useEffect(() => {
    // Fetch all stocks for search autocomplete
    fetch('/api/stocks')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setAllStocks(data);
      })
      .catch(err => console.error('Failed to load stocks list for search', err));
  }, []);
  ...
  const handleSelectTicker = (targetCode: string) => {
    const cleanCode = targetCode.trim().toUpperCase();
    if (!cleanCode) return;
    setShowDropdown(false);
    setSearchQuery('');
    if (onSelectStock) {
      onSelectStock(cleanCode);
    }
  };
  ...
  const renderSearchBar = () => (
    <div ref={searchContainerRef} className="bg-[#14161B] p-4 rounded-xl border border-gray-800 space-y-3 relative shadow-lg">
        ...
        <div className="flex-1 relative">
          <form onSubmit={handleSearchSubmit} className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Cari Ticker Saham IDX (contoh: BBCA, BBRI, TLKM, PGAS, DOOH, PANI)..."
              className="w-full bg-[#0E1014] border border-gray-800 text-white placeholder-gray-500 text-xs md:text-sm rounded-xl pl-10 pr-24 py-2.5 focus:outline-none focus:border-emerald-500/60 font-mono font-bold tracking-wider transition-all"
            />
            {searchQuery && (
              <button
                type="button"
                onClick={() => { setSearchQuery(''); setShowDropdown(false); }}
                className="absolute right-20 text-gray-400 hover:text-white p-1 cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            )}
            <button
              type="submit"
              className="absolute right-1.5 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold rounded-lg text-xs transition-all cursor-pointer flex items-center gap-1 shadow-md shadow-emerald-500/20"
            >
              Cari <ChevronRight className="w-3.5 h-3.5" />
            </button>
          </form>

          {/* Autocomplete Dropdown List */}
          {showDropdown && filteredStocks.length > 0 && (
            <div className="absolute left-0 right-0 top-full mt-2 bg-[#0E1014] border border-gray-800 rounded-xl shadow-2xl z-50 max-h-64 overflow-y-auto divide-y divide-gray-850/60">
              {filteredStocks.map((s) => (
                <button
                  key={s.code}
                  type="button"
                  onClick={() => handleSelectTicker(s.code)}
                  className="w-full text-left px-4 py-2.5 hover:bg-emerald-500/10 flex items-center justify-between group transition-colors cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    <span className="font-mono font-bold text-emerald-400 group-hover:text-emerald-300 text-sm">{s.code}</span>
                    <span className="text-xs text-gray-400 truncate max-w-[180px] sm:max-w-xs">{s.name}</span>
                  </div>
                  <div className="flex items-center gap-2.5 font-mono text-xs">
                    <span className="text-gray-300">Rp {s.price.toLocaleString()}</span>
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${s.changePercent >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs no-scrollbar">
        <span className="text-[11px] text-gray-500 font-mono shrink-0">Akses Cepat Ticker:</span>
        {['BBCA', 'BBRI', 'BMRI', 'TLKM', 'ASII', 'PGAS', 'PANI', 'DOOH', 'GOTO', 'ANTM'].map((code) => (
          <button
            key={code}
            type="button"
            onClick={() => handleSelectTicker(code)}
            className={`px-2.5 py-1 rounded-lg text-[11px] font-mono font-bold transition-all cursor-pointer shrink-0 border ${
              stockCode === code
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        {renderSearchBar()}
        <div className="p-16 text-center space-y-4 bg-[#14161B] border border-gray-800 rounded-xl">
          <div className="w-10 h-10 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto"></div>
          <p className="text-sm text-slate-400 font-medium">Mengambil data analisis teknikal & chart {stockCode}...</p>
        </div>
      </div>
    );
  }
